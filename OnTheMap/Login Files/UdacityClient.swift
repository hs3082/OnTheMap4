...
class UdacityClient {
    ...
    class func postStudentLoaction(PostPinLocation: PostPinLocation, completion: @escaping (PostPinLocationResponse?, Error?) -> Void) {
        
        var request = URLRequest(url: Endpoints.postStudentLocation.url)
        request.httpMethod = "POST"
        request.addValue("application/json", forHTTPHeaderField: "Content-Type")
        
        let body = PostPinLocation
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            let decoder = JSONDecoder()
            do {
                let responseObject = try decoder.decode(PostPinLocationResponse.self, from: data)
                DispatchQueue.main.async {
    ...
    class func putStudentLocation(objectID: String, PostPinLocation: PostPinLocation, completion: @escaping (Bool, Error?) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            do {
                let responseObj = try decoder.decode(PostPinLocationResponse.self, from: data)
                DispatchQueue.main.async {
                     print("\(responseObj)")
                     completion(true, nil)
                }
